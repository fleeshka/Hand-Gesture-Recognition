...
while True:
    ...
    if result.multi_hand_landmarks and result.multi_handedness:
        ...
        smooth_latency = sum(latency_history) / len(latency_history)
        latency_text = f"{smooth_latency*1000:.1f} ms"

        gesture_text = le.inverse_transform([pred_class])[0]

    cv2.putText(frame_mirrored, f"Gesture: {gesture_text}",
                (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (200, 200, 0), 2)
    ...
    cv2.putText(frame_mirrored, f"FPS: {smooth_fps:.1f}",
                (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 200, 0), 2)
    cv2.putText(frame_mirrored, f"Latency: {latency_text}",
                (10, 160), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 200, 200), 2)
