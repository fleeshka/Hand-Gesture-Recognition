...
# Gesture classes (update these based on your model)
GESTURE_CLASSES = [
    "thumbs_up", "thumbs_down", "peace", "ok", "fist",
    "open_palm", "point", "rock", "wave", "stop"
]
...
def preprocess_image(image_bytes: bytes, target_size: tuple = (224, 224)) -> Optional[np.ndarray]:
...
def predict_gesture(image_bytes: bytes) -> dict:
    ...
    try:
        ...
        for i, prob in enumerate(predictions[0]):
            class_name = GESTURE_CLASSES[i] if i < len(GESTURE_CLASSES) else f"class_{i}"
...
def demo_prediction() -> dict:
    ...
    for g in demo_gestures:
        if g == gesture:
            ...
            all_predictions.append({"class": g, "confidence": round(random.uniform(0.01, 0.3), 2)})
...
async def predict_gesture_endpoint(file: UploadFile = File(...)):
    ...
    try:
        ...
        log.info(f"Prediction: {result['gesture']} (confidence: {result['confidence']:.2f})")
