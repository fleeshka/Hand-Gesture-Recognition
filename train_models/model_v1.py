...
def main():
    ...
    gesture_model = GestureModel(num_classes=6)

    print("\nLoading data...")
    X, y = gesture_model.load_and_preprocess_data(csv_files)

    print(f"Loaded {len(X)} samples")
    print(f"Feature dimension: {X.shape}")  # Should be (n_samples, 64)
    ...
    print(
        f"  Right hand samples: {np.sum(hand_feature == 1)} ({np.mean(hand_feature == 1)*100:.1f}%)"
    )
    print(
        f"  Left hand samples: {np.sum(hand_feature == 0)} ({np.mean(hand_feature == 0)*100:.1f}%)"
    )
    ...
    print(
        f"Accuracy for right hand: {right_accuracy:.4f} ({np.sum(right_mask)} samples)"
    )
    ...
    for idx in sample_indices:
        ...
        print(
            f"True: {true_label:10} Predicted: {pred_label:10} Hand: {hand_type:5} {'✓' if is_correct else '✗'}"
        )
